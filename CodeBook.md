...
- activlabel ( read from 'activity_labels.txt'): list if activity names- 6 rows, 2 columns (colums2: name of acitivites)

- xtrain (read from X_train.txt'): Training set- 7352 rows (observatioan), 561 columns (measurements)
...
- subjtrain (read from 'subject_train.txt): 7352 rows (observations), 1 column (subject ID). Each row identifies the subject who performed the activity for each window sample. Its range is from 1 to 30. 
